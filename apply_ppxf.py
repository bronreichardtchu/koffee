...
def main(lamdas, data_flat, noise_flat, xx_flat, yy_flat, ssp_filepath, z, results_folder, galaxy_name, cube_colour, fwhm_gal=2.0, fwhm_temp=2.0, em_lines=True, fwhm_emlines=2.0, degree=4, mdegree=0, vacuum=True, extra_em_lines=False, tie_balmer=True, plot=False, quiet=False):
    ...
    Inputs:
        lamdas: wavelength vector for galaxy data
        data_flat: 2d array of the data with shape [lamdas, pixels]
        noise_flat: 2d array of the noise (square root of variance) with shape [lamdas, pixels]
        xx_flat:
        yy_flat:
        ssp_filepath: (string) filepath leading to the SSP files
        z: (float) the redshift of the galaxy
        results_folder:
        galaxy_name:
        cube_colour: which cube is being fit 'red' or 'blue'
        fwhm_gal:
        em_lines: (boolean) whether or not to include emission lines in the fit
        degree:
        mdegree:
        vacuum: whether to make wavelengths in vacuum or air wavelengths.  (default=True, default in ppxf is False)
        extra_em_lines : bool
            set to True to include extra emission lines often found in
            KCWI data (OII 4317, [OIII]4363, OII4414 and [NeIII]3868).
            (default=False)
    ...
    #make arrays to save results into
    if em_lines == True:
        pp_sol = np.zeros((gal_logspec.shape[1],6))
        pp_error = np.zeros((gal_logspec.shape[1],6))
        pp_gas_reddening = np.zeros_like(gal_logspec[0,:])
        pp_weights = np.zeros((gal_logspec.shape[1], templates.shape[1]))
        pp_bestfit = np.zeros_like(gal_logspec)
        pp_gas_bestfit = np.zeros_like(pp_bestfit)
        pp_chi2 = np.zeros_like(gal_logspec[0,:])
        pp_cont_subtracted = np.zeros_like(gal_logspec)
        pp_galaxy = np.zeros_like(gal_logspec)
        if cube_colour == 'red':
            pp_flux_ratio = np.zeros_like(gal_flux_ratio)
    else:
        pp_sol = np.zeros((gal_logspec.shape[1],4))
        pp_error = np.zeros((gal_logspec.shape[1],4))
        pp_weights = np.zeros((gal_logspec.shape[1], templates.shape[1]))
        pp_bestfit = np.zeros_like(gal_logspec)
        pp_chi2 = np.zeros_like(gal_logspec[0,:])
        pp_cont_subtracted = np.zeros_like(gal_logspec)
    ...
    #run ppxf
    for i, (logspec, lognoise, velscale) in enumerate(zip(gal_logspec.T, log_noise.T, gal_velscale)):
        ...
        if em_lines == True:
            ...
            #save the results
            pp_sol[i, :2] = pp.sol[0]
            pp_sol[i, 2:4] = pp.sol[1]
            pp_sol[i, 4:] = pp.sol[2]
            pp_error[i, :2] = pp.error[0]
            pp_error[i, 2:4] = pp.error[1]
            pp_error[i, 4:] = pp.error[2]
            pp_gas_reddening[i] = pp.gas_reddening
            pp_weights[i, :] = pp.weights
            pp_bestfit[:, i] = pp.bestfit
            pp_gas_bestfit[:, i] = pp.gas_bestfit
            pp_chi2[i] = pp.chi2
            pp_cont_subtracted[:, i] = pp.galaxy - (pp.bestfit-pp.gas_bestfit)
            pp_galaxy[:, i] = pp.galaxy
            if cube_colour == 'red':
                pp_flux_ratio[i] = cont_subt_flux_ratio
            ...
            pp_bestfit[:, i] = pp.bestfit
            pp_chi2[i] = pp.chi2
            pp_cont_subtracted[:, i] = pp.galaxy - pp.bestfit
            pp_galaxy[:, i] = pp.galaxy
            if cube_colour == 'red':
                pp_flux_ratio[i] = cont_subt_flux_ratio
    ...
    with open(results_folder+galaxy_name+'_ppxf_solutions_errors', 'wb') as f:
        pickle.dump([pp_sol, pp_error], f)
    f.close()

    with open(results_folder+galaxy_name+'_ppxf_weights', 'wb') as f:
        pickle.dump(pp_weights, f)
    f.close()

    with open(results_folder+galaxy_name+'_ppxf_bestfit', 'wb') as f:
        pickle.dump(pp_bestfit, f)
    f.close()

    with open(results_folder+galaxy_name+'_ppxf_chi2', 'wb') as f:
        pickle.dump(pp_chi2, f)
    f.close()

    with open(results_folder+galaxy_name+'_ppxf_continuum_subtracted', 'wb') as f:
        pickle.dump([pp.lam, pp_galaxy, pp_cont_subtracted], f)
    f.close()

    with open(results_folder+galaxy_name+'_ppxf_continuum_subtracted_unnormalised', 'wb') as f:
        pickle.dump([pp.lam, gal_norm, pp_galaxy*gal_norm, pp_cont_subtracted*gal_norm], f)
    f.close()

    if cube_colour == 'red':
        with open(results_folder+galaxy_name+'_ppxf_continuum_subtracted_flux_ratio', 'wb') as f:
            pickle.dump(pp_flux_ratio, f)
        f.close()

    #with open(results_folder+galaxy_name+'_flux_ratio', 'wb') as f:
    #    pickle.dump(gal_flux_ratio, f)
    #f.close()

    if em_lines == True:
        with open(results_folder+galaxy_name+'_ppxf_gas_bestfit', 'wb') as f:
            pickle.dump(pp_gas_bestfit, f)
        f.close()

        with open(results_folder+galaxy_name+'_ppxf_gas_reddening', 'wb') as f:
            pickle.dump(pp_gas_reddening, f)
        f.close()

    print('==========FINISHED==========')

    if em_lines == True:
        return pp_sol, pp_error, pp_weights, pp_bestfit, pp_chi2, pp_gas_bestfit, pp_gas_reddening, pp_galaxy, pp_cont_subtracted, pp.lam
    else:
        return pp_sol, pp_error, pp_weights, pp_bestfit, pp_chi2, pp_galaxy, pp_cont_subtracted, pp.lam
