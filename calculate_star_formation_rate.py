...
def calc_sfr_koffee(outflow_results, outflow_error, no_outflow_results, no_outflow_error, statistical_results, z, include_extinction=True, include_outflow=False):
    ...
    if include_outflow == False:
        results = np.full((3, statistical_results.shape[0], statistical_results.shape[1]), np.nan, dtype=np.double)
        error = np.full((3, statistical_results.shape[0], statistical_results.shape[1]), np.nan, dtype=np.double)
    ...
    if include_outflow == False:
        ...
        results[3:,~flow_mask] = np.zeros((3, statistical_results.shape[0], statistical_results.shape[1]))[:,~flow_mask]
        results[:,flow_mask] = outflow_results[:6, flow_mask]

        error[:3,~flow_mask] = no_outflow_error[:3, ~flow_mask]
        error[3:,~flow_mask] = np.zeros((3, statistical_results.shape[0], statistical_results.shape[1]))[:,~flow_mask]
